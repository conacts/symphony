...
name: linear
description: |
  Prefer Linear MCP for routine issue and comment work; use Symphony's
  `linear_graphql` client tool only for raw GraphQL operations that MCP
  does not expose cleanly.
---

# Linear

Use Linear MCP first for routine issue reads, state changes, comments, links,
and metadata updates.

Use the `linear_graphql` fallback in Symphony app-server sessions only when:

- Linear MCP is unavailable in the current session, or
- you need a raw GraphQL mutation, upload flow, or introspection path that MCP
  does not expose cleanly.
...
For routine Symphony progress updates, prefer short status-oriented comments:
...
Keep deep technical rationale in commits, PR descriptions, and review replies.
