defmodule SymphonyElixir.StatusDashboard do
  ...
  defp format_snapshot_content(snapshot_data, tps, terminal_columns_override \\ nil) do
    case snapshot_data do
      {:ok, %{running: running, retrying: retrying, codex_totals: codex_totals} = snapshot} ->
        rate_limits = Map.get(snapshot, :rate_limits)
        project_link_lines = format_scope_lines()
  ...
  defp format_scope_lines do
    scope_line =
      case Config.linear_scope() do
        {:project, project_slug} ->
          colorize("│ Scope: ", @ansi_bold) <> colorize(linear_project_url(project_slug), @ansi_cyan)

        {:team, team_key} ->
          excluded_count =
            Config.settings!().tracker.excluded_project_ids
            |> Enum.uniq()
            |> length()

          summary =
            "team #{team_key} (excluding #{excluded_count} project#{if excluded_count == 1, do: "", else: "s"})"

          colorize("│ Scope: ", @ansi_bold) <> colorize(summary, @ansi_cyan)

        :missing ->
          colorize("│ Scope: ", @ansi_bold) <> colorize("n/a", @ansi_gray)
      end
