defmodule SymphonyElixir.Codex.AppServer do
  ...
  def start_session(workspace, opts \\ []) do
    ...
    with {:ok, expanded_workspace} <- validate_workspace_cwd(workspace, worker_host),
         {:ok, session_policies} <- session_policies(expanded_workspace, issue, worker_host),
         {:ok, port} <- start_port(expanded_workspace, session_policies.command, worker_host) do
  ...
  defp session_policies(workspace, issue, nil) do
    Config.codex_runtime_settings(workspace, issue: issue)
  end

  defp session_policies(workspace, issue, worker_host) when is_binary(worker_host) do
