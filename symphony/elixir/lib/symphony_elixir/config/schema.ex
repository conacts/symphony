defmodule SymphonyElixir.Config.Schema do
  ...
  defmodule Tracker do
    ...
    embedded_schema do
      ...
      field(:excluded_project_ids, {:array, :string}, default: [])
      field(:assignee, :string)
      field(:dispatchable_states, {:array, :string}, default: ["Todo", "In Progress"])
      field(:terminal_states, {:array, :string}, default: ["Closed", "Canceled", "Duplicate", "Done"])
      field(:claim_transition_to_state, :string)
      field(:claim_transition_from_states, {:array, :string}, default: [])
      field(:startup_failure_transition_to_state, :string)
    end
    ...
    def changeset(schema, attrs) do
      ...
      |> cast(
        ...
        [
          ...
          :dispatchable_states,
          :terminal_states,
          :claim_transition_to_state,
          :claim_transition_from_states,
          :startup_failure_transition_to_state
        ],
        empty_values: []
      )
    end
  ...
  defmodule GitHub do
    ...
    embedded_schema do
      ...
      field(:allowed_review_logins, {:array, :string}, default: [])
      field(:allowed_rework_comment_logins, {:array, :string}, default: [])
    end
  ...
  defp finalize_settings(settings) do
    tracker = %{
      ...
      | api_key: resolve_secret_setting(settings.tracker.api_key, System.get_env("LINEAR_API_KEY")),
        ...
        team_key: normalize_optional_string(settings.tracker.team_key),
        excluded_project_ids: normalize_string_list(settings.tracker.excluded_project_ids),
    ...
    github = %{
      settings.github
      | webhook_secret: resolve_secret_setting(settings.github.webhook_secret, System.get_env("GITHUB_WEBHOOK_SECRET")),
        api_token: resolve_secret_setting(settings.github.api_token, System.get_env("GITHUB_TOKEN")),
        state_path: resolve_path_value(settings.github.state_path, default_github_state_path()),
        allowed_review_logins: normalize_login_list(settings.github.allowed_review_logins),
        allowed_rework_comment_logins: normalize_login_list(settings.github.allowed_rework_comment_logins)
    }

    %{settings | tracker: tracker, workspace: workspace, codex: codex, github: github}
  end

  defp migrate_legacy_tracker_keys(%{"tracker" => tracker} = config) when is_map(tracker) do
    cond do
      Map.has_key?(tracker, "dispatchable_states") ->
        config

      Map.has_key?(tracker, "active_states") ->
        put_in(config, ["tracker", "dispatchable_states"], Map.get(tracker, "active_states"))
  ...
  defp migrate_legacy_tracker_keys(config), do: config
  ...
  defp normalize_login_list(_values), do: []
  ...
  defp default_github_state_path do
    Path.expand("~/.symphony/state/github-review-events.ndjson")
  end
