defmodule SymphonyElixir.LiveE2ETest do
  ...
  defp dispatchable_state_names(%{"states" => %{"nodes" => states}}) when is_list(states) do
    states
    |> Enum.map(& &1["name"])
    |> Enum.reject(fn
      name when is_binary(name) ->
        String.downcase(name) in ["backlog", "blocked", "in review", "done", "canceled", "cancelled", "duplicate"]

      _ ->
        true
    end)
    |> case do
      [] -> ["Todo", "In Progress", "Rework", "Approved"]
      names -> names
    end
  end
  ...
  defp run_live_issue_flow!(backend) when backend in [:local, :ssh] do
    ...
    try do
      ...
      write_workflow_file!(workflow_file,
        ...
        tracker_dispatchable_states: dispatchable_state_names(team),
