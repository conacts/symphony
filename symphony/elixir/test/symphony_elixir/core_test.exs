defmodule SymphonyElixir.CoreTest do
  ...
  test "config defaults and validation checks" do
    ...
    assert config.tracker.dispatchable_states == ["Todo", "In Progress"]
    assert config.tracker.terminal_states == ["Closed", "Canceled", "Duplicate", "Done"]
    assert config.tracker.claim_transition_to_state == nil
    assert config.tracker.claim_transition_from_states == []
    assert config.tracker.startup_failure_transition_to_state == nil
    ...
    write_workflow_file!(Workflow.workflow_file_path(),
      ...
      tracker_excluded_project_ids: ["project-id"]
    )

    assert {:error, {:invalid_workflow_config, message}} = Config.validate!()
    assert message =~ "tracker.excluded_project_ids"

    write_workflow_file!(Workflow.workflow_file_path(),
      tracker_project_slug: "project",
      tracker_excluded_project_ids: [],
      codex_command: ""
    )
    ...
    write_workflow_file!(Workflow.workflow_file_path(),
      tracker_claim_transition_from_states: ["Todo"]
    )

    assert {:error, {:invalid_workflow_config, message}} = Config.validate!()
    assert message =~ "tracker.claim_transition_to_state"

    write_workflow_file!(Workflow.workflow_file_path(),
      tracker_startup_failure_transition_to_state: "Todo"
    )

    assert {:error, {:invalid_workflow_config, message}} = Config.validate!()
    assert message =~ "tracker.startup_failure_transition_to_state"
  ...
  test "current WORKFLOW.md file is valid and complete" do
    ...
    assert Map.get(tracker, "dispatchable_states") == ["Todo", "In Progress", "Rework"]
    assert Map.get(tracker, "claim_transition_to_state") == "In Progress"
    assert Map.get(tracker, "claim_transition_from_states") == ["Todo", "Rework"]
    ...
    assert is_map(codex)
    assert Map.get(codex, "command") =~ "--model gpt-5.4 app-server"

    assert String.trim(prompt) != ""
    assert prompt =~ "Prefer Linear MCP tools for reading and updating Linear issues"
    assert prompt =~ "Repo-local skills may be available in `.codex/skills`."
    assert prompt =~ "`vercel` CLI to inspect deployment or preview failures"
    assert prompt =~ "The canonical branch for this ticket is `symphony/{{ issue.identifier }}`."
    assert prompt =~ "`Rework`: review feedback or follow-up changes are needed"
    assert prompt =~ "`Blocked`: waiting on an external blocker or human input"
    assert prompt =~ "`In Review`: waiting for human review or decision"
    assert prompt =~ "Leave short Linear status comments for major milestones"
    assert prompt =~ "Keep deep technical rationale in commits, the PR body, and PR review replies"
    assert prompt =~ "Create the first PR once there is a reviewable slice of work"
    assert prompt =~ "Symphony does not merge PRs in this workflow."
    refute prompt =~ "`Needs Agent Update`"
  ...
  test "SymphonyElixir.start_link delegates to the orchestrator" do
    write_workflow_file!(Workflow.workflow_file_path(),
      tracker_kind: "memory",
      tracker_startup_failure_transition_to_state: "Backlog"
    )
  ...
  test "non-active issue state stops running agent without cleaning workspace" do
    ...
    workspace = Path.join(test_root, "symphony-" <> issue_identifier)
  ...
  test "missing running issues stop active agents without cleaning the workspace" do
    ...
    try do
      ...
      assert {:ok, workspace} =
               SymphonyElixir.PathSafety.canonicalize(Path.join(test_root, "symphony-" <> issue_identifier))
  ...
  test "prepare_issue_for_dispatch_for_test transitions configured source states before dispatch" do
    ...
    write_workflow_file!(Workflow.workflow_file_path(),
      tracker_kind: "memory",
      tracker_active_states: ["Todo", "In Progress", "Rework"],
      tracker_claim_transition_to_state: "In Progress",
      tracker_claim_transition_from_states: ["Todo", "Rework"]
    )
    ...
    issue = %Issue{
      ...
      identifier: "MT-562",
      state: "Rework",
      title: "Resume review changes",
      description: "Pick up review feedback",
      labels: []
    }

    assert {:ok, prepared_issue} = Orchestrator.prepare_issue_for_dispatch_for_test(issue)
    assert prepared_issue.state == "In Progress"
    assert_receive {:memory_tracker_state_update, "issue-transition", "In Progress"}
    assert_receive {:memory_tracker_comment, "issue-transition", comment_body}
    assert comment_body =~ "Symphony status update."
    assert comment_body =~ "State: `In Progress`"
    assert comment_body =~ "moved it from `Rework` to `In Progress`"
    assert comment_body =~ "Branch: `symphony/MT-562`"
  end

  test "prepare_issue_for_dispatch_for_test leaves non-source states unchanged" do
    previous_recipient = Application.get_env(:symphony_elixir, :memory_tracker_recipient)
    ...
    issue = %Issue{
      id: "issue-no-transition",
      identifier: "MT-563",
      state: "In Progress",
      title: "Already active",
      description: "No transition required",
      labels: []
    }

    assert {:ok, prepared_issue} = Orchestrator.prepare_issue_for_dispatch_for_test(issue)
    assert prepared_issue.state == "In Progress"
    refute_receive {:memory_tracker_state_update, "issue-no-transition", _}
    refute_receive {:memory_tracker_comment, "issue-no-transition", _}
  end
  ...
  test "agent failure message records the original startup error for the running issue" do
    issue_id = "issue-startup-failure"
    reason = {:workspace_hook_failed, "after_create", 1, "QSTASH_TOKEN is required\n"}
    ...
    assert {:noreply, updated_state} =
             Orchestrator.handle_info({:agent_failure, issue_id, reason}, state)

    assert updated_state.running[issue_id].last_failure_reason == reason
  end

  test "startup failures create a single issue comment with a useful excerpt" do
    previous_recipient = Application.get_env(:symphony_elixir, :memory_tracker_recipient)
    ...
    assert Config.settings!().tracker.startup_failure_transition_to_state == "Backlog"

    issue_id = "issue-startup-comment"
    first_ref = make_ref()
    second_ref = make_ref()
    orchestrator_name = Module.concat(__MODULE__, :StartupFailureCommentOrchestrator)
    {:ok, pid} = Orchestrator.start_link(name: orchestrator_name)
    ...
    failure_reason =
      {:workspace_hook_failed, "after_create", 1,
       """
       From https://github.com/conacts/coldets-v2
       > coldets-v2@ local:prepare /home/cs/junction/coldets-v2
       QSTASH_TOKEN is required in /home/cs/junction/coldets-v2/.env.machine.local for local-core runtime preparation.
    ...
    :sys.replace_state(pid, fn _ ->
      initial_state
      |> Map.put(:running, %{issue_id => first_running_entry})
      |> Map.put(:claimed, MapSet.new([issue_id]))
      |> Map.put(:retry_attempts, %{})
    end)

    send(pid, {:agent_failure, issue_id, failure_reason})
    send(pid, {:DOWN, first_ref, :process, self(), :boom})

    assert_receive {:memory_tracker_state_update, ^issue_id, "Backlog"}, 200
    assert_receive {:memory_tracker_comment, ^issue_id, first_comment}, 200
    assert first_comment =~ "Symphony agent startup failed."
    assert first_comment =~ "workspace hook `after_create` exited with status 1."
    assert first_comment =~ "QSTASH_TOKEN is required"
    assert first_comment =~ "Symphony did not retry automatically."
    assert first_comment =~ "Symphony moved the issue to `Backlog`."

    first_state = :sys.get_state(pid)
    refute Map.has_key?(first_state.retry_attempts, issue_id)
    assert MapSet.member?(first_state.claimed, issue_id)

    second_running_entry = %{
      pid: self(),
      ref: second_ref,
      identifier: "MT-563",
      retry_attempt: 1,
    ...
    :sys.replace_state(pid, fn state ->
      state
      |> Map.put(:running, %{issue_id => second_running_entry})
      |> Map.put(:claimed, MapSet.new([issue_id]))
      |> Map.put(:retry_attempts, %{})
    end)

    send(pid, {:agent_failure, issue_id, failure_reason})
    send(pid, {:DOWN, second_ref, :process, self(), :boom})

    assert_receive {:memory_tracker_state_update, ^issue_id, "Backlog"}, 200
    refute_receive {:memory_tracker_comment, ^issue_id, _second_comment}, 200
  end

  test "max-turn completions leave a Linear comment before scheduling continuation" do
    previous_recipient = Application.get_env(:symphony_elixir, :memory_tracker_recipient)
    ...
    issue_id = "issue-max-turn-comment"
    process_ref = make_ref()
    orchestrator_name = Module.concat(__MODULE__, :MaxTurnCommentOrchestrator)
    {:ok, pid} = Orchestrator.start_link(name: orchestrator_name)
    ...
    running_entry = %{
      ...
      ref: process_ref,
      identifier: "MT-611",
      issue: %Issue{id: issue_id, identifier: "MT-611", state: "In Progress"},
      last_rate_limits: %{
        limit_id: "gpt-5",
    ...
    send(pid, {:agent_max_turns_reached, issue_id, 2})
    send(pid, {:DOWN, process_ref, :process, self(), :normal})

    assert_receive {:memory_tracker_comment, ^issue_id, comment_body}, 200
    assert comment_body =~ "Symphony agent paused after reaching max turns."
    assert comment_body =~ "configured 2-turn limit"
    assert comment_body =~ "Latest rate limits: gpt-5 | primary 0/20000 reset 95s | secondary 0/60 reset 45s"
    assert comment_body =~ "Symphony will start a fresh run automatically while the issue remains in an active state."

    updated_state = :sys.get_state(pid)
    assert Map.has_key?(updated_state.retry_attempts, issue_id)
  end

  test "rate-limit failures leave a Linear comment with the latest limit snapshot" do
    previous_recipient = Application.get_env(:symphony_elixir, :memory_tracker_recipient)
    ...
    issue_id = "issue-rate-limit-comment"
    process_ref = make_ref()
    orchestrator_name = Module.concat(__MODULE__, :RateLimitCommentOrchestrator)
    {:ok, pid} = Orchestrator.start_link(name: orchestrator_name)
    ...
    running_entry = %{
      ...
      ref: process_ref,
      identifier: "MT-612",
      issue: %Issue{id: issue_id, identifier: "MT-612", state: "In Progress"},
      last_rate_limits: %{
        limit_id: "gpt-5",
        primary: %{remaining: 0, limit: 20_000, reset_in_seconds: 95},
        secondary: %{remaining: 0, limit: 60, reset_in_seconds: 45}
    ...
    send(
      pid,
      {:agent_failure, issue_id, {:turn_failed, %{"message" => "Rate limit exceeded", "code" => "rate_limit_exceeded"}}}
    )

    send(pid, {:DOWN, process_ref, :process, self(), :boom})

    assert_receive {:memory_tracker_comment, ^issue_id, comment_body}, 200
    assert comment_body =~ "Symphony agent paused after hitting a Codex rate limit."
    assert comment_body =~ "Codex hit a rate limit and ended the current run."
    assert comment_body =~ "Latest rate limits: gpt-5 | primary 0/20000 reset 95s | secondary 0/60 reset 45s"
    assert comment_body =~ "Symphony will retry automatically after backoff while the issue remains in an active state."

    updated_state = :sys.get_state(pid)
    assert Map.has_key?(updated_state.retry_attempts, issue_id)
  end

  test "parked startup-failure claims release after the issue leaves active states" do
    ...
    issue_id = "issue-parked-claim"
    issue = %Issue{id: issue_id, identifier: "MT-564", state: "Backlog"}
    ...
    state = %Orchestrator.State{
      ...
      running: %{},
      retry_attempts: %{},
      failure_comment_signatures: %{issue_id => "signature"},
      codex_totals: %{input_tokens: 0, output_tokens: 0, total_tokens: 0, seconds_running: 0}
    }

    updated_state = Orchestrator.reconcile_parked_claims_for_test(state)

    refute MapSet.member?(updated_state.claimed, issue_id)
    refute Map.has_key?(updated_state.failure_comment_signatures, issue_id)
  end
  ...
  test "repo-root WORKFLOW.md includes approved merge handoff" do
    workflow_path = Workflow.workflow_file_path()
    ...
    assert {:ok, %{config: config, prompt: prompt}} = Workflow.load(repo_workflow_path)

    tracker = Map.get(config, "tracker", %{})
    assert Map.get(tracker, "active_states") == ["Todo", "In Progress", "Rework", "Approved"]
    assert Map.get(tracker, "team_key") == "COL"
    assert is_list(Map.get(tracker, "excluded_project_ids"))
    assert Map.get(tracker, "claim_transition_to_state") == "In Progress"
    assert Map.get(tracker, "claim_transition_from_states") == ["Todo", "Rework"]
    assert Map.get(tracker, "startup_failure_transition_to_state") == "Backlog"

    assert Config.settings!().tracker.dispatchable_states == ["Todo", "In Progress", "Rework", "Approved"]
    assert Config.settings!().tracker.team_key == "COL"
    assert Config.max_concurrent_agents_for_state("Approved") == 1
    assert prompt =~ "If `{{ issue.state }}` is `Approved`, this is a merge-only session:"
    assert prompt =~ "Leave a brief **Linear status note** that the approved merge session has started"
    assert prompt =~ "git push --force-with-lease"
    assert prompt =~ "gh pr merge symphony/{{ issue.identifier }} --squash --delete-branch"
    assert prompt =~ "move the issue back to `In Review`"
    assert prompt =~ "move the issue to `Done`"
    assert prompt =~ "`symphony:no-auto-rework`"
    assert prompt =~ "`symphony:disabled`"
  end
  ...
  test "agent runner keeps workspace after successful codex run" do
    ...
    try do
      ...
      created =
        MapSet.difference(entries_after, before) |> Enum.filter(&(&1 == "symphony-S-99"))

      created = MapSet.new(created)

      assert MapSet.size(created) == 1
      workspace_name = created |> Enum.to_list() |> List.first()
      assert workspace_name == "symphony-S-99"
    ...
    try do
      ...
      case "$*" in
        ...
        *worker-b*"__SYMPHONY_WORKSPACE__"*)
          printf '%s\\t%s\\t%s\\n' '__SYMPHONY_WORKSPACE__' '1' '/remote/home/.symphony-remote-workspaces/symphony-MT-SSH-FAILOVER'
          exit 0
          ;;
        *)
          exit 0
  ...
  test "app server startup command supports codex args override from workflow config" do
    ...
    try do
      ...
      write_workflow_file!(Workflow.workflow_file_path(),
        workspace_root: workspace_root,
        codex_command: "#{codex_binary} --model gpt-5.4 app-server"
      )
      ...
      assert argv_line = Enum.find(lines, fn line -> String.starts_with?(line, "ARGV:") end)
      assert String.contains?(argv_line, "--model gpt-5.4 --config model_reasoning_effort=xhigh app-server")
