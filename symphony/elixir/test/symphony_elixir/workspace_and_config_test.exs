defmodule SymphonyElixir.WorkspaceAndConfigTest do
  ...
  test "before_run hook can fail closed unless the workspace is a linked git worktree" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-elixir-worktree-validation-#{System.unique_integer([:positive])}"
      )

    repo_root = Path.expand("../../../..", __DIR__)
    validator = Path.join(repo_root, "scripts/symphony/validate-worktree.sh")

    try do
      source_repo = Path.join(test_root, "source")
      workspace_root = Path.join(test_root, "workspaces")
      valid_workspace = Path.join(workspace_root, "symphony-COL-200")
      invalid_workspace = Path.join(workspace_root, "symphony-COL-201")
      ...
      System.cmd("git", ["-C", source_repo, "worktree", "add", "-b", "symphony/COL-200", valid_workspace, "HEAD"])

      File.mkdir_p!(Path.join(valid_workspace, ".symphony"))
      File.write!(Path.join(valid_workspace, ".symphony/workspace.env"), "COLDETS_SYMPHONY_DATABASE_URL=postgres://example\n")

      File.mkdir_p!(invalid_workspace)
      File.mkdir_p!(Path.join(invalid_workspace, ".symphony"))
      File.write!(Path.join(invalid_workspace, ".symphony/workspace.env"), "COLDETS_SYMPHONY_DATABASE_URL=postgres://example\n")

      hook_command =
        "COLDETS_SYMPHONY_SOURCE_REPO=#{source_repo} #{validator} \"$PWD\""

      write_workflow_file!(Workflow.workflow_file_path(),
        workspace_root: workspace_root,
        hook_before_run: hook_command
      )

      assert :ok = Workspace.run_before_run_hook(valid_workspace, "COL-200")

      assert {:error, {:workspace_hook_failed, "before_run", 1, output}} =
               Workspace.run_before_run_hook(invalid_workspace, "COL-201")

      assert output =~ "not a git checkout" or output =~ "not a linked git worktree"
    after
      File.rm_rf(test_root)
    end
  end
  ...
  test "workspace path is deterministic per issue identifier" do
    ...
    assert Path.basename(first_workspace) == "symphony-MT_Det"
  end
  ...
  test "repo-owned workspace reuses an existing validated worktree without rerunning after_create" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-elixir-repo-owned-workspace-reuse-#{System.unique_integer([:positive])}"
      )

    previous_source_repo = System.get_env("COLDETS_SYMPHONY_SOURCE_REPO")

    on_exit(fn ->
      restore_env("COLDETS_SYMPHONY_SOURCE_REPO", previous_source_repo)
    end)

    try do
      source_repo = Path.join(test_root, "source")
      workspace_root = Path.join(test_root, "workspaces")
      workspace = Path.join(workspace_root, "symphony-COL-302")
      ...
      System.cmd("git", ["-C", source_repo, "add", "README.md"])
      System.cmd("git", ["-C", source_repo, "commit", "-m", "initial"])
      System.cmd("git", ["-C", source_repo, "worktree", "add", "-b", "symphony/COL-302", workspace, "HEAD"])

      File.mkdir_p!(Path.join(workspace, ".symphony"))

      File.write!(
        Path.join(workspace, ".symphony/workspace.env"),
        "COLDETS_SYMPHONY_DATABASE_URL=postgres://example\n"
      )

      System.put_env("COLDETS_SYMPHONY_SOURCE_REPO", source_repo)

      write_workflow_file!(Workflow.workflow_file_path(),
        workspace_root: workspace_root,
        hook_after_create: "echo should-not-run > after_create.txt"
      )

      assert {:ok, canonical_workspace} = SymphonyElixir.PathSafety.canonicalize(workspace)
      assert {:ok, reused_workspace} = Workspace.create_for_issue("COL-302")
      assert reused_workspace == canonical_workspace
      assert File.exists?(Path.join(reused_workspace, ".symphony/workspace.env"))
      refute File.exists?(Path.join(reused_workspace, "after_create.txt"))
    after
      File.rm_rf(test_root)
    end
  end

  test "repo-owned workspace resets stale directories missing metadata before rerunning after_create" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-elixir-repo-owned-workspace-reset-#{System.unique_integer([:positive])}"
      )
    ...
    try do
      source_repo = Path.join(test_root, "source")
      workspace_root = Path.join(test_root, "workspaces")
      stale_workspace = Path.join(workspace_root, "symphony-COL-303")
      ...
      File.write!(Path.join(stale_workspace, "README.md"), "stale\n")
      File.write!(Path.join(stale_workspace, "local-progress.txt"), "keep me if reused\n")

      System.put_env("COLDETS_SYMPHONY_SOURCE_REPO", source_repo)

      write_workflow_file!(Workflow.workflow_file_path(),
        workspace_root: workspace_root,
        hook_after_create: "mkdir -p .symphony && echo COLDETS_SYMPHONY_DATABASE_URL=postgres://example > .symphony/workspace.env && echo bootstrapped > README.md"
      )

      assert {:ok, canonical_workspace} = SymphonyElixir.PathSafety.canonicalize(stale_workspace)
      assert {:ok, workspace} = Workspace.create_for_issue("COL-303")
      assert workspace == canonical_workspace
      assert File.read!(Path.join(workspace, "README.md")) == "bootstrapped\n"
      assert File.exists?(Path.join(workspace, ".symphony/workspace.env"))
  ...
  test "workspace replaces stale non-directory paths" do
    ...
    try do
      stale_workspace = Path.join(workspace_root, "symphony-MT-STALE")
      File.mkdir_p!(workspace_root)
  ...
  test "workspace rejects symlink escapes under the configured root" do
    ...
    try do
      ...
      symlink_path = Path.join(workspace_root, "symphony-MT-SYM")
  ...
  test "workspace canonicalizes symlinked workspace roots before creating issue directories" do
    ...
    try do
      ...
      assert {:ok, canonical_workspace} =
               SymphonyElixir.PathSafety.canonicalize(Path.join(actual_root, "symphony-MT-LINK"))
  ...
  test "workspace creates an empty directory when no bootstrap hook is configured" do
    ...
    try do
      ...
      workspace = Path.join(workspace_root, "symphony-MT-608")
      assert {:ok, canonical_workspace} = SymphonyElixir.PathSafety.canonicalize(workspace)
  ...
  test "workspace removes all workspaces for a closed issue identifier" do
    ...
    try do
      target_workspace = Path.join(workspace_root, "symphony-S_1")
  ...
  test "linear client normalizes blockers from inverse relations" do
    raw_issue = %{
      ...
      "team" => %{"key" => "COL"},
      "project" => %{"id" => "project-1", "name" => "Coldets Inbox", "slugId" => "coldets-inbox"},
      "assignee" => %{
        "id" => "user-1"
      },
    ...
    assert issue.team_key == "COL"
    assert issue.project_id == "project-1"
    assert issue.project_name == "Coldets Inbox"
    assert issue.project_slug == "coldets-inbox"
  ...
  test "linear client fetches a single issue by identifier via direct issue query" do
    raw_issue = %{
      "id" => "issue-96",
      "identifier" => "COL-96",
      "title" => "Webhook publication",
      "description" => "desc",
      "priority" => 3,
      "state" => %{"name" => "In Review"},
      "branchName" => "symphony/COL-96",
      "url" => "https://linear.app/coldets/issue/COL-96/test",
      "assignee" => nil,
      "labels" => %{"nodes" => []},
    ...
    graphql_fun = fn query, variables ->
      assert query =~ "query SymphonyLinearIssueByIdentifier"
      assert variables == %{id: "COL-96", relationFirst: 50}
      {:ok, %{"data" => %{"issue" => raw_issue}}}
    end

    assert {:ok, %Issue{} = issue} = Client.fetch_issue_by_identifier_for_test("COL-96", graphql_fun)
    assert issue.identifier == "COL-96"
    assert issue.state == "In Review"
    assert issue.branch_name == "symphony/COL-96"
  end
  ...
  test "linear client paginates issue state fetches by id beyond one page" do
    ...
    raw_issue = fn issue_id ->
      ...
      %{
        ...
        "state" => %{"name" => "In Progress"},
        "team" => %{"key" => "COL"},
        "project" => %{"id" => "project-1", "name" => "Project", "slugId" => "project"},
        "labels" => %{"nodes" => []},
        "inverseRelations" => %{"nodes" => []}
      }
    end
  ...
  test "linear client filters team-scoped issues by project exclusion and symphony disable label" do
    issue_ids = ["issue-1", "issue-2", "issue-3"]

    graphql_fun = fn query, variables ->
      assert query =~ "SymphonyLinearIssuesById"
      assert variables == %{ids: issue_ids, first: 3, relationFirst: 50}
           ...
           "issues" => %{
             "nodes" => [
               %{
                 ...
                 "state" => %{"name" => "Todo"},
                 "team" => %{"key" => "COL"},
                 "project" => %{"id" => "project-keep", "name" => "Keep", "slugId" => "keep"},
                 "labels" => %{"nodes" => []},
                 "inverseRelations" => %{"nodes" => []}
               },
               %{
                 "id" => "issue-2",
                 "identifier" => "COL-2",
                 "title" => "Excluded project issue",
                 "state" => %{"name" => "Todo"},
                 "team" => %{"key" => "COL"},
                 "project" => %{"id" => "project-excluded", "name" => "Draft", "slugId" => "draft"},
                 "labels" => %{"nodes" => []},
                 "inverseRelations" => %{"nodes" => []}
               },
               %{
                 ...
                 "team" => %{"key" => "COL"},
                 "project" => %{"id" => "project-keep", "name" => "Keep", "slugId" => "keep"},
                 "labels" => %{"nodes" => [%{"name" => "symphony:disabled"}]},
                 "inverseRelations" => %{"nodes" => []}
               }
             ]
           }
    ...
    write_workflow_file!(Workflow.workflow_file_path(),
      ...
      tracker_team_key: "COL",
      tracker_excluded_project_ids: ["project-excluded"]
    )
  ...
  test "config reads defaults for optional settings" do
    ...
    assert config.tracker.excluded_project_ids == []
    ...
    assert Config.settings!().codex.command == "codex app-server --model gpt-5.4"
    ...
    assert message =~ "tracker.dispatchable_states"
  ...
  test "config resolves $VAR references for env-backed secret and path values" do
    ...
    github_secret_env_var = "SYMP_GITHUB_WEBHOOK_SECRET_#{System.unique_integer([:positive])}"
    github_token_env_var = "SYMP_GITHUB_TOKEN_#{System.unique_integer([:positive])}"
    github_state_path_env_var = "SYMP_GITHUB_STATE_PATH_#{System.unique_integer([:positive])}"
    workspace_root = Path.join("/tmp", "symphony-workspace-root")
    api_key = "resolved-secret"
    github_secret = "resolved-webhook-secret"
    github_token = "resolved-github-token"
    github_state_path = Path.join("/tmp", "github-review-events.ndjson")
    codex_bin = Path.join(["~", "bin", "codex"])

    previous_workspace_root = System.get_env(workspace_env_var)
    previous_api_key = System.get_env(api_key_env_var)
    previous_github_secret = System.get_env(github_secret_env_var)
    previous_github_token = System.get_env(github_token_env_var)
    previous_github_state_path = System.get_env(github_state_path_env_var)

    System.put_env(workspace_env_var, workspace_root)
    System.put_env(api_key_env_var, api_key)
    System.put_env(github_secret_env_var, github_secret)
    System.put_env(github_token_env_var, github_token)
    System.put_env(github_state_path_env_var, github_state_path)

    on_exit(fn ->
      restore_env(workspace_env_var, previous_workspace_root)
      restore_env(api_key_env_var, previous_api_key)
      restore_env(github_secret_env_var, previous_github_secret)
      restore_env(github_token_env_var, previous_github_token)
      restore_env(github_state_path_env_var, previous_github_state_path)
    end)
    ...
    write_workflow_file!(Workflow.workflow_file_path(),
      ...
      github_webhook_secret: "$#{github_secret_env_var}",
      github_api_token: "$#{github_token_env_var}",
      github_state_path: "$#{github_state_path_env_var}",
    ...
    assert config.github.repo == "conacts/coldets-v2"
    assert config.github.webhook_secret == github_secret
    assert config.github.api_token == github_token
    assert config.github.state_path == Path.expand(github_state_path)
  ...
  test "schema parse normalizes policy keys and env-backed fallbacks" do
    ...
    empty_github_secret_env = "SYMP_EMPTY_GITHUB_SECRET_#{System.unique_integer([:positive])}"
    missing_github_state_env = "SYMP_MISSING_GITHUB_STATE_#{System.unique_integer([:positive])}"
    ...
    previous_empty_github_secret_env = System.get_env(empty_github_secret_env)
    previous_missing_github_state_env = System.get_env(missing_github_state_env)
    previous_linear_api_key = System.get_env("LINEAR_API_KEY")
    previous_github_webhook_secret = System.get_env("GITHUB_WEBHOOK_SECRET")

    System.delete_env(missing_workspace_env)
    System.put_env(empty_secret_env, "")
    System.delete_env(missing_secret_env)
    System.put_env(empty_github_secret_env, "")
    System.delete_env(missing_github_state_env)
    System.put_env("LINEAR_API_KEY", "fallback-linear-token")
    System.put_env("GITHUB_WEBHOOK_SECRET", "fallback-github-secret")
    ...
    on_exit(fn ->
      ...
      restore_env(empty_github_secret_env, previous_empty_github_secret_env)
      restore_env(missing_github_state_env, previous_missing_github_state_env)
      restore_env("LINEAR_API_KEY", previous_linear_api_key)
      restore_env("GITHUB_WEBHOOK_SECRET", previous_github_webhook_secret)
    end)

    assert {:ok, settings} =
             Schema.parse(%{
               tracker: %{api_key: "$#{empty_secret_env}"},
               github: %{webhook_secret: "$#{empty_github_secret_env}"},
               workspace: %{root: "$#{missing_workspace_env}"},
               codex: %{approval_policy: %{reject: %{sandbox_approval: true}}}
             })

    assert settings.tracker.api_key == nil
    assert settings.github.webhook_secret == nil
    ...
    assert {:ok, settings} =
             Schema.parse(%{
               ...
               github: %{state_path: "$#{missing_github_state_env}"},
               workspace: %{root: ""}
             })

    assert settings.tracker.api_key == "fallback-linear-token"
    assert settings.github.state_path == Path.expand("~/.symphony/state/github-review-events.ndjson")
    assert settings.workspace.root == Path.join(System.tmp_dir!(), "symphony_workspaces")

    assert {:ok, settings} =
             Schema.parse(%{
               github: %{webhook_secret: "$MISSING_GITHUB_WEBHOOK_SECRET"}
             })

    assert settings.github.webhook_secret == "fallback-github-secret"
  end
  ...
  test "codex runtime settings apply default and label-driven launch overrides" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-elixir-runtime-codex-labels-#{System.unique_integer([:positive])}"
      )
    ...
    try do
      ...
      write_workflow_file!(Workflow.workflow_file_path(),
        workspace_root: workspace_root,
        codex_command: "codex --config shell_environment_policy.inherit=all app-server --model gpt-5.4"
      )

      default_issue = %Issue{labels: []}

      assert {:ok, runtime_settings} =
               Config.codex_runtime_settings(issue_workspace, issue: default_issue)

      assert runtime_settings.model == "gpt-5.4"
      assert runtime_settings.reasoning_effort == "xhigh"

      assert runtime_settings.command ==
               "codex --config shell_environment_policy.inherit=all --model gpt-5.4 --config model_reasoning_effort=xhigh app-server"

      override_issue = %Issue{
        labels: [
          "frontend",
      ...
      assert {:ok, runtime_settings} =
               Config.codex_runtime_settings(issue_workspace, issue: override_issue)

      assert runtime_settings.model == "gpt-5.4-mini"
      assert runtime_settings.reasoning_effort == "high"

      assert runtime_settings.command ==
               "codex --config shell_environment_policy.inherit=all --model gpt-5.4-mini --config model_reasoning_effort=high app-server"

      spark_issue = %Issue{
        labels: ["symphony:model:gpt-5.3-codex-spark"]
      }

      assert {:ok, runtime_settings} =
               Config.codex_runtime_settings(issue_workspace, issue: spark_issue)

      assert runtime_settings.model == "gpt-5.3-codex-spark"
      assert runtime_settings.reasoning_effort == "xhigh"

      assert runtime_settings.command ==
               "codex --config shell_environment_policy.inherit=all --model gpt-5.3-codex-spark --config model_reasoning_effort=xhigh app-server"
    after
      File.rm_rf(test_root)
    end
  end

  test "codex runtime settings fail closed on conflicting or unsupported label overrides" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-elixir-runtime-codex-label-errors-#{System.unique_integer([:positive])}"
      )

    try do
      workspace_root = Path.join(test_root, "workspaces")
      issue_workspace = Path.join(workspace_root, "MT-103")
      File.mkdir_p!(issue_workspace)
      ...
      assert {:error, {:invalid_issue_label_override, :model, {:conflicting, ["gpt-5.4", "gpt-5.4-mini"]}}} =
               Config.codex_runtime_settings(issue_workspace, issue: conflicting_issue)

      unsupported_issue = %Issue{
        labels: ["symphony:reasoning:turbo"]
      }

      assert {:error, {:invalid_issue_label_override, :reasoning_effort, {:unsupported, "turbo", ["low", "medium", "high", "xhigh"]}}} =
               Config.codex_runtime_settings(issue_workspace, issue: unsupported_issue)
    after
      File.rm_rf(test_root)
    end
  end
  ...
  test "remote workspace lifecycle uses ssh host aliases from worker config" do
    ...
    try do
      ...
      workspace_path = "/remote/home/.symphony-remote-workspaces/symphony-MT-SSH-WS"
      ...
      assert trace =~ "~/.symphony-remote-workspaces/symphony-MT-SSH-WS"
