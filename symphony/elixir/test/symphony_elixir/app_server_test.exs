defmodule SymphonyElixir.AppServerTest do
  ...
  test "app server launch command applies issue label model and reasoning overrides" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-elixir-app-server-label-overrides-#{System.unique_integer([:positive])}"
      )
    ...
    try do
      ...
      trace_file = Path.join(test_root, "codex-label-overrides.trace")
      ...
      #!/bin/sh
      trace_file="${SYMP_TEST_CODEx_TRACE:-/tmp/codex-label-overrides.trace}"
      count=0
      printf 'ARGV:%s\\n' "$*" >> "$trace_file"
      ...
      write_workflow_file!(Workflow.workflow_file_path(),
        workspace_root: workspace_root,
        codex_command: "#{codex_binary} app-server --model gpt-5.4"
      )

      issue = %Issue{
        id: "issue-label-overrides",
        identifier: "MT-1002",
        title: "Validate label-driven codex profile",
        description: "Ensure issue labels control the app-server launch profile",
        state: "In Progress",
        url: "https://example.org/issues/MT-1002",
        labels: ["symphony:model:gpt-5.3-codex-spark", "symphony:reasoning:high"]
      }

      assert {:ok, _result} = AppServer.run(workspace, "Use the label-driven codex profile", issue)

      trace = File.read!(trace_file)
      lines = String.split(trace, "\n", trim: true)

      assert argv_line = Enum.find(lines, fn line -> String.starts_with?(line, "ARGV:") end)
      assert String.contains?(argv_line, "--model gpt-5.3-codex-spark --config model_reasoning_effort=high app-server")
      refute String.contains?(argv_line, "--model gpt-5.4 app-server")
    after
      File.rm_rf(test_root)
    end
  end
  ...
  test "app server launches over ssh for remote workers" do
    ...
    try do
      ...
      assert argv_line =~ "fake-remote-codex --model gpt-5.4 --config model_reasoning_effort=xhigh app-server"
