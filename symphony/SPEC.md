...
- `project_slug` (string)
  ...
  - Use for single-project Linear scope.
- `team_key` (string)
  - Optional when `tracker.kind == "linear"`.
  - Use for team-scoped Linear dispatch.
- `excluded_project_ids` (list of strings)
  - Optional.
  - Valid only with `team_key`.
  - Team-scoped polling should exclude matching Linear project IDs after issue fetch.
...
- one of `tracker.project_slug` or `tracker.team_key` is present when required by the selected
  tracker kind.
...
- `tracker.project_slug`: string, optional single-project scope when `tracker.kind=linear`
- `tracker.team_key`: string, optional team scope when `tracker.kind=linear`
- `tracker.excluded_project_ids`: list of strings, optional, valid with `tracker.team_key`
...
- `tracker.project_slug` maps to Linear project `slugId` in single-project mode
- `tracker.team_key` maps to Linear team `key` in team-scoped mode
- Candidate issue query filters either:
  - project using `project: { slugId: { eq: $projectSlug } }`, or
  - team using `team: { key: { eq: $teamKey } }`
- Team-scoped issue admission must:
  - fetch project metadata on each issue,
  - reject issues with no project membership,
  - reject issues whose project ID appears in `tracker.excluded_project_ids`
...
- `project_id`, `project_name`, `project_slug`, and `team_key` should be preserved when available
